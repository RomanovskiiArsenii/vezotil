...
export default function Header() {
    ...
    const navItems = useMemo(
        () => [
            ...
            { label: 'Новости', href: '/news', type: 'link' },
            {
                label: (
                    <>
                        Отзывы <span className="custom-nav-hide">врачей</span>
                    </>
                ),
                href: '/video',
                type: 'link',
    ...
    return (
        <motion.header
            ...
            style={{ zIndex: 100 }}
        >
            <div className="container mx-auto px-4 lg:px-8 xl:px-12 max-w-6xl custom-overflow-visible">
                <div className="flex items-center justify-between h-20 custom-overflow-visible">
                    ...
                    <nav className="hidden xl:flex items-center space-x-1 flex-1 justify-center">
                        {navItems.map((item, index) => (
                            <motion.div
                                ...
                                // href идеальный ключ для навигационного меню: он всегда уникален внутри массива и стабилен
                                // key={item.label}
                            ...
                            >
                                {item.type === 'link' ? (
                                    ...
                                    >
                                        <span className="custom-nav-item whitespace-pre-line">{item.label}</span>
                                    </Link>
                                ) : (
                                    <button
                    ...
                    <nav className="hidden lg:flex xl:hidden items-center space-x-1 flex-1 justify-center">
                        {navItems.slice(0, 6).map((item, index) => (
                            <motion.div
                                ...
                                animate={{ opacity: 1, y: 0 }}
                                transition={{ duration: 0.3, delay: index * 0.03 }}
                                className="custom-overflow-y-hide"
                            >
                                {item.type === 'link' ? (
                                    <Link
                                        href={item.href}
                                        prefetch={true}
                                        className="custom-nav-item-wrap-sm px-2 py-2 rounded-lg text-gray-300 hover:text-white hover:bg-blue-600/20 transition-all duration-300 font-medium text-center text-xs leading-tight"
                                    >
                                        <span className="custom-nav-item-sm whitespace-pre-line">{item.label}</span>
                                    </Link>
                                ) : (
                                    <button
                                        onClick={() => handleNavClick(item)}
                    ...
                    <motion.div
                        ...
                        className="hidden lg:block custom-overflow-visible"
                    >
                        <button
                    ...
                    {/* Mobile Menu Button */}
                    <button onClick={() => setIsOpen(!isOpen)} className="lg:hidden p-2 hud-element rounded-lg">
                        {isOpen ? <X className="w-6 h-6 hud-text" /> : <Menu className="w-6 h-6 hud-text" />}
                    </button>
                </div>
                ...
                >
                    <nav className="custom-nav-modal py-4 space-y-2 bg-slate-900/95 rounded-lg mt-2">
                        {navItems.map((item) => (
                            <div key={item.href}>
                                {/* label больше не строка и его нельзя применить как ключ */}
                                {/* href идеальный ключ для навигационного меню: он всегда уникален внутри массива и */}
                                {/* <div key={item.label}> */}
